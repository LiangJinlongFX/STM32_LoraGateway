/**
  ******************************************************************************
  * @file    STM32F407DEMO/sdio_sdcard.c 
  * @author  Liang
  * @version V1.0.0
  * @date    2017-4-26
  * @brief   
  ...
  * @attention
	* 加入超时判断,解决轮询接收死机的问题.
	* 新增SD_GetState和SD_SendStatus函数.
	* 支持SDHC,MMC卡
	*！！！由于SD初始化函数干扰串口发送函数（原因未明）对于SDHC卡将频率降到6MHz后恢复正常	
  ******************************************************************************
  */
...
SD_Error SD_PowerON(void)
{
  ...
  SDIO_Init(&SDIO_InitStructure);//初始化配置
  ...
  SDIO->CLKCR|=1<<8;			//SDIOCK使能  
	...
	{
		//SDIO指令录入配置
		SDIO_CmdInitStructure.SDIO_Argument = 0x00;									//发送CMD0进入IDLE STAGE模式命令.
    ...
    SDIO_SendCommand(&SDIO_CmdInitStructure);	  								//写命令进命令寄存器
 	...
 	if(errorstatus)return errorstatus;//指令无响应，返回错误状态
